					...
					<div class="col-md-12">
						<div class="row">
							<div class="form-group col-md-5">
								...
								$customer = $conn->query("SELECT * FROM customer_list order by customer_name asc");
						...
						<div class="edit row mb-3">
								<div class="col-md-4">
									...
									while($row=$product->fetch_assoc()):
										$prod[$row['id']] = $row;
									?>
										<option value="<?php echo $row['id'] ?>" data-name="<?php echo $row['name'] ?>" data-description="<?php echo $row['description'] ?>"> <?php echo $row['name'] . ' | ' . $row['batch']?> </option>
									<?php endwhile; ?>
									</select>
								</div>
								<div class="col-md-2">
							...
							<table class="table table-bordered" id="list">
								...
								?>
										...
										<tr class="item-row">
											...
											<td>
												<input type="number" min="1" step="any" name="qty[]" value="<?php echo $invent[$row['id']]['qty'] ?>" class="text-right">
											</td>
											<td>
												<input type="hidden" min="1" step="any" name="price[]" value="<?php echo $prod[$row['product_id']]['selling_price']  ?>" class="text-right">
						...
						<div class="row">
							<button class="edit btn btn-primary btn-sm btn-block float-right " type="button" id="pay">Save</button>
						</div>
					</div>
					<div class="modal fade" id="pay_modal" role='dialog'>
					    <div class="modal-dialog modal-md" role="document">
					      ...
					      <div class="modal-footer">
					        <button type="button" class="btn btn-primary" id='submit' onclick="$('#manage-sales').submit()">View Receipt</button>
			...
			$.ajax({
				...
				success:function(resp){
					...
					if(resp.available >= qty){
						...
						tr.find('.price').html(resp.selling_price)
						tr.find('[name="product_id[]"]').val(product)
						tr.find('[name="qty[]"]').val(qty)
						tr.find('[name="price[]"]').val(resp.selling_price)
